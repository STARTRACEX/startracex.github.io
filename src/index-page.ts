...
export class IndexPage extends LitElement {
  @property() avatar: string = "https://avatars.githubusercontent.com/u/94100555?v=4";
...
<main>
  ...
  <nav>
    <flex-flow>
    ${this.sites.map((site) => {
  return html`<super-a arrow="delta" href="//${site}/">${site}</super-a>`;
})}
    </flex-flow>
  </nav>
</main>
  ...
  protected firstUpdated(_changedProperties: PropertyValueMap<any> | Map<PropertyKey, unknown>): void {
    const logo = '  _____ _______       _____ _______ _____            _____ ______ \n / ____|__   __|/\\   |  __ \\__   __|  __ \\     /\\   / ____|  ____|\n| (___    | |  /  \\  | |__) | | |  | |__) |   /  \\ | |    | |__   \n \\___ \\   | | / /\\ \\ |  _  /  | |  |  _  /   / /\\ \\| |    |  __|  \n ____) |  | |/ ____ \\| | \\ \\  | |  | | \\ \\  / ____ \\ |____| |____ \n|_____/   |_/_/    \\_\\_|  \\_\\ |_|  |_|  \\\\_\/_/    \\_\\_____|______|';
    console.log(logo);
    fetch("https://api.github.com/users/startracex/repos").then(res => res.json()).then(res => {
      this.repos = res.map((repo: any) => {
        return html`<flex-flow flexflow="column" style="margin: .25em;">
  <super-a arrow="delta" href="${repo.html_url}">${repo.name}</super-a>
